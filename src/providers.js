 ...
 * @private
 */
function writeProviderEntry (store, cid, peer, time) {
  const dsKey = [
    makeProviderKey(cid),
    '/',
  ...
  return store.put(new Key(dsKey), Buffer.from(varint.encode(time)))
}
...
async function loadProviders (store, cid) {
  ...
  const it = store.query({ prefix: makeProviderKey(cid) })
  for await (const entry of it) {
