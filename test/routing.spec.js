...
function createPeerId (n) {
  return Promise.all([...new Array(n)].map(() => PeerId.create({ bits: 512 })))
}
...
describe('Routing Table', () => {
  ...
  it('add', async function () {
    ...
    await Promise.all([...new Array(1000)].map(() => {
      return table.add(ids[random(ids.length - 1)])
    }))

    await Promise.all([...new Array(20)].map(async () => {
  ...
  it('closestPeers', async function () {
    ...
    const peers = await createPeerId(19)
